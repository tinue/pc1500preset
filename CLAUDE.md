...
- `src/`, `docs/`, `samples/`, `scripts/` are this project's own code —
  free to edit.
- `vendor/pc1500emu/` is a git submodule (`vendor/pc1500emu.git`, pinned
  commit, currently `master`). **Never modify it from work done in this
  project.** This tool's entire design point is driving an unmodified
  upstream binary through its own FIFO interface rather than patching it —
  see the README's "Why this is a separate project" section. If a task
  seems to need a change inside `vendor/pc1500emu/`, that's a signal the
  task belongs in a separate session/checkout of `pc1500emu` itself, not
  here.
- That said, `vendor/pc1500emu/` is a real, independently-developed repo
  with its own `.claude/skills/pc1500-dev/SKILL.md` (LH5801 CPU internals,
  ...
  touches files under `vendor/pc1500emu/` — you don't need to invoke it
  manually.
...
- **`tasm-preprocessor`** — adding real preprocessor/macro-expander support
  ...
  `vendor/pc1500emu`'s `pc1500disasm --mode convert` (TASM → sdas
  converter). This is `vendor/pc1500emu` source, so per the boundary above
...
./scripts/build_all.sh                          # also builds vendor/pc1500emu
```

No automated test suite exists for `src/` yet (unlike `vendor/pc1500emu`,
...
vendor/pc1500emu/   submodule, never modified here
```
